...
```{r}
age_population_data <- read_csv(here("data", "UV103_age_health_board_census.csv"), skip = 10) %>% # locates the csv and excludes the first 10 lines in the csv as they are redundant
...
age_population_data <- age_population_data %>% 
  filter(!age%in% c("All people", "Under 1", "85 and over")) %>% 
  mutate(age = as.numeric(age)) %>% 
  mutate(agegroup = case_when(between(age, 0,15)~ '15andunder',
                              between(age, 16,34)~'16-35',
                              age >35 ~ '35andover'))

age_population_data <- age_population_data %>% 
  filter(age%in% c(16:34)) %>% 
  mutate(age = as.numeric(age)) %>% 
  mutate(agegroup = case_when(between(age, 16,34)~'16-34'))
```
