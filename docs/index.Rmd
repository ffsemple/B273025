...
This plot explores if there is monthly variation in prescribing rate of ECP for the years 2019 to 2022. I included 4 years of consecutive data to see if a one-time event, such as the Covid-19 pandemic or the Common Wealth Games caused any changes in ECP prescribing rate. 
...
This figure explores if there are differences in the average annual prescription rate for each Health Board in Scotland. Additionally I calculated the proportion of young people within each health board to see if there was an association. 
...
This figure explores if quantity of prescriptions varies by SIMD. Corrected for SIMD having different number of GPs. 
...
Generally as SIMD decile increases the number of prescriptions of ECP by GP decreases. GPs in SIMD1, the most deprived areas, prescribe the most ECP whilst increasing affluence is related to decreasing number of ECP prescribed. This suggests those in deprived areas are utilising ECP more, highlighting an inequality in health need. Perhaps this disparity is due to education, safe practises, environmental factors.
...
Differences may suggest patients access health services later, so have to use Ulipristal Acetate, or that GPs or Pharmacists have differences in prescribing preferences in more deprived areas.
